...
int main(int argc, const char *argv[]) {
    ...
    while (cleanup != 1) {
        ...
        listenfd = create_udp_sock(&B_addr, &A_addr, B_RECEIVE_PORT_NUM);
        ...
        if (sel <= 0) {
            ...
            if (FD_ISSET(listenfd, &readfds) || FD_ISSET(listenfd, &errorfds)) {
                ...
                do {
                    ...
                    bytesRcvd = (int)recvfrom(listenfd, (char*)connection_buf, BUFSIZE, MSG_PEEK, (struct sockaddr*)&A_addr, &A_addr_len);
                } while (bytesRcvd <= 0);

                if (bytesRcvd > 0) {
                    if(strcmp(inet_ntoa(A_addr.sin_addr),A_IP_ADDR) != 0) {
                        /* setup for connect */
                        memset(&A_addr, 0, sizeof(A_addr));
                    	A_addr.sin_family = AF_INET;
                    	A_addr.sin_addr.s_addr = inet_addr(A_IP_ADDR);
                    	A_addr.sin_port = htons(A_SEND_PORT_NUM);
                    }
                    /* connect the UDP socket to the port A_PORT_NUM and to the A machine's IP address */
                    if(connect(listenfd, (struct sockaddr *)&A_addr, sizeof(A_addr)) != 0) {
                		perror("connect()");
                		cleanup = 1;
                        break;
                	} else {
                ...
                do {
                    ...
                    bytesRcvd = (int)recvfrom(listenfd, (char *)&tmp_buf, sizeof(tmp_buf), 0, (struct sockaddr*)&A_addr, &A_addr_len);
                } while (bytesRcvd <= 0);
